...
class Shades:
    ...
    def __init__(self, name, offCode, onCode, stopCode, iot, workQueue):
        self.name = name
        self.offCode = offCode
        self.onCode = onCode
    ...
    def set(self, state):
        print ('Setting new state for: %s using Thread: %s' % (self.name, threading.currentThread().getName())
...
def transmit_code(code):
    print ('Transmitting: %s using Thread: %s' % (code, threading.currentThread().getName())
...
if __name__ == "__main__":
    try:
        ...
        Shades('study-room-shade', sr_ch1_off, sr_ch1_on, sr_ch1_stop, iot, workQueue).set('OFF')
        

        print('Listening...')

        while True:
            code = workQueue.get(True, None)
            transmit_code(code)
